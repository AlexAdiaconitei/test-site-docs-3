...
function materializeDocusaurusFrontmatter(frontmatter: Frontmatter) {
  ...
  if (frontmatter.label) next.sidebar_label = frontmatter.label;
  if (frontmatter.order) next.sidebar_position = frontmatter.order;
  if (frontmatter.hidden === "true") next.unlisted = "true";

  return next;
}

function materializeDocusaurusMarkdown(content: string) {
...
async function materializeDocusaurusMarkdownFiles(directory: string) {
